...
/**
 * Restlet responsible for REST server-side authentication for bearer and apikey schemes.
 */
public class RestServerAuthenticationRestlet extends Restlet {
    ...
    private final HttpAuthenticationSpec authentication;
    private final Restlet next;
    private final Set<String> allowedVariables;

    public RestServerAuthenticationRestlet(HttpAuthenticationSpec authentication, Restlet next) {
    ...
    private boolean authenticateBearer(Request request) {
        if (!(authentication instanceof HttpBearerAuthenticationSpec bearerAuth)) {
            return false;
        }
    ...
    private boolean authenticateApiKey(Request request) {
        if (!(authentication instanceof HttpApiKeyAuthenticationSpec apiKeyAuth)) {
            return false;
        }
