...
exposed operation (REST or MCP) that performs multiple dependent calls in sequence
...
- chain multiple consumed operations into one exposed operation (REST endpoint or MCP tool);
...
- the goal is only to “compose several independent calls into one response” with no ordering/dependency (still possible here, but `compose-apis` is the better story framing).
