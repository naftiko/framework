...
| Situation | Action |
|---|---|
| "I want to combine several APIs into a single reusable service" | Read `references/compose-apis.md` |
| "I want agents to use this API as MCP tools, resources, or prompts" | Read `references/wrap-api-as-mcp.md` |
| "I want to ship an API wrapper today and refine it incrementally" | Read `references/proxy-then-customize.md` |
| "I want to chain API calls into a single declared operation" | Read `references/chain-api-calls.md` |
...
3. **Fill consumes.** For each external API: set `type: "http"`, a unique
   `namespace` (kebab-case), `baseUri` (no trailing slash), `resources`,
   and `operations`. Add `authentication` if needed.
4. **Fill exposes.** Choose the adapter type (REST, MCP, or Skill) and
...
2. Every `consumes` entry must have both `baseUri` and `namespace`.
   Namespaces must be unique across all consumes entries.
3. Every `exposes` entry must have a `namespace`. Namespaces must be
   unique across all exposes entries.
4. At least one of `exposes` or `consumes` must be present in `capability`.
5. `call` references must follow `{namespace}.{operationName}` and point
   to a valid consumed operation.
6. `{exposeNamespace}.{paramName}` is the only syntax for referencing
   exposed input parameters in `with` injectors — do not invent alternatives.
7. `variable` expressions resolve from `binds` keys.
8. `ForwardConfig` requires `targetNamespace` (single string, not array)
   and `trustedHeaders` (at least one entry).
9. MCP tools must have `name` and `description`. MCP tool input parameters
   must have `name`, `type`, and `description`.
10. ExposedOperation supports exactly two modes (oneOf): simple (`call` +
    optional `with`) or orchestrated (`steps` + optional `mappings`). Never
    mix fields from both modes.
11. Do not modify `scripts/lint-capability.sh` unless explicitly asked —
    it wraps Spectral with the correct ruleset and flags.
12. Do not add properties that are not in the JSON Schema — the schema
    uses `additionalProperties: false` on most objects.
13. Every exposed `inputParameter` must be referenced in at least one
    step's `with` block or mapping — orphan parameters bloat the API
    surface and confuse consumers.
14. Every consumed `outputParameter` must be referenced in the exposed
    part (via mappings, or `outputParameters`) — unreferenced
    outputs are dead declarations that add noise without value.
15. Do not prefix variable names with the capability, namespace, or
